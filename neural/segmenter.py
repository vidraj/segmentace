...
def create_encoder_layers(layer_count, layer_input, input_lengths, rnn_dim, char_dim, dropout_keep_prob, scope):
    ...
    (source_encoded_fwd, source_encoded_bwd), (source_states_fwd, source_states_bwd) = tf.nn.bidirectional_dynamic_rnn(encoder_rnn_cell_fwd, encoder_rnn_cell_bwd, layer_input, sequence_length=input_lengths, dtype=tf.float32, swap_memory=True, scope="{}_0".format(scope))
    ...
    for layer_id in range(1, layer_count):
        ...
        output, states = tf.nn.dynamic_rnn(encoder_rnn_cell, output, sequence_length=input_lengths, dtype=tf.float32, swap_memory=True, scope="{}_{}".format(scope, layer_id))
...
class Network:
    ...
    def construct(self, args, train_size, source_chars, target_chars, target_seg_chars, bow, eow):
        with self.session.graph.as_default():
            ...
            with tf.variable_scope("encoder"):
                ...
                source_encoded, source_states = create_encoder_layers(args.encoder_layers, source_embedded, self.source_seq_lens, args.rnn_dim, args.char_dim, self.dropout_keep_prob, scope="rnn_encoder")
            ...
            # Decoder
            with tf.variable_scope("parent_decoder"):
                ...
                class DecoderTraining(tf.contrib.seq2seq.Decoder):
                    def __init__(self, decoder, output_layer, source_states, target_embeddings, bow, eow):
                        self.decoder = decoder
                        self.decoder_layer = output_layer
                    ...
                    @property
                    def batch_size(self): return tf.shape(self.source_states)[0] # Return size of the batch, using for example source_states size
                    @property
                    ...
                    def initialize(self, name=None):
                        ...
                        states = self.decoder.set_initial_state(self.source_states, self.batch_size) # Initial decoder state to use
                        inputs = with_attention(tf.nn.embedding_lookup(self.target_embeddings, tf.fill([self.batch_size], self.bow)), states) # Call with_attention on the embedded BOW characters of shape [self.batch_size].
                    ...
                    def step(self, time, inputs, states, name=None):
                        ...
                        outputs, states = self.decoder(inputs) # Run the decoder GRU cell using inputs and states.
                        outputs = self.decoder_layer(outputs) # Apply the decoder_layer on outputs.
                        predicted_output = tf.argmax(outputs, axis=-1, output_type=tf.int32)
                        ni = (1 - gold_predict_ratio) * tf.nn.embedding_lookup(self.target_embeddings, predicted_output) + gold_predict_ratio * tf.gather(target_embedded, time, axis=1)
                ...
                output_layer, _, _ = tf.contrib.seq2seq.dynamic_decode(DecoderTraining(decoder, decoder_layer, source_states, target_embeddings, bow, eow), swap_memory=True)
                ...
                class DecoderPrediction(tf.contrib.seq2seq.Decoder):
                    def __init__(self, decoder, decoder_layer, source_states, target_embeddings, bow, eow):
                        self.decoder = decoder
                    ...
                    def step(self, time, inputs, states, name=None):
                        ...
                        next_input = with_attention(tf.nn.embedding_lookup(self.target_embeddings, outputs), states) # Embed `outputs` using target_embeddings and pass it to with_attention.
                        finished = tf.equal(outputs, self.eow) # True where outputs==eow, False otherwise
                        return outputs, states, next_input, finished
                self.predicted_lemmas, _, self.predicted_lemma_lengths = tf.contrib.seq2seq.dynamic_decode(
                    DecoderPrediction(decoder, decoder_layer, source_states, target_embeddings, bow, eow),
            ...
            # Predict the segmentations directly from the encoder.
            with tf.variable_scope("segment_decoder"):
                segment_decoder_is_rnn = True
                if segment_decoder_is_rnn:
                    #segment_rnn_cell = tf.nn.rnn_cell.GRUCell(num_units=target_seg_chars, activation=None)
                    #segment_output, segment_states = tf.nn.dynamic_rnn(segment_rnn_cell, source_encoded, sequence_length=seg_lens, dtype=tf.float32, swap_memory=True, scope="segment_predict_layer")
                    segment_output, segment_states = create_encoder_layers(args.segmenter_layers, source_encoded, seg_lens, args.rnn_dim, args.char_dim, self.dropout_keep_prob, scope="rnn_segmenter")
                    segment_predict_layer = tf.layers.Dense(target_seg_chars, activation=None, name="segment_predict_layer")
                    segment_output = segment_predict_layer(segment_output)
                else:
                    segment_predict_layer = tf.layers.Dense(target_seg_chars, activation=None, name="segment_predict_layer")
            ...
            parent_loss = tf.losses.sparse_softmax_cross_entropy(target_seqs, output_layer, weights=weights)
            loss = parent_loss + seg_loss
            ...
            self.training_joint = tf.train.AdamOptimizer(learning_rate).minimize(loss, global_step=global_step, var_list=tf.global_variables("encoder") + tf.global_variables("parent_decoder") + tf.global_variables("segment_decoder"), name="training_joint")
            self.training_segment = tf.train.AdamOptimizer(learning_rate).minimize(seg_loss, global_step=global_step, var_list=tf.global_variables("encoder") + tf.global_variables("segment_decoder"), name="training_segment")
            self.training_parent = tf.train.AdamOptimizer(learning_rate).minimize(parent_loss, global_step=global_step, var_list=tf.global_variables("encoder") + tf.global_variables("parent_decoder"), name="training_parent")
    ...
    def train_epoch(self, train, batch_size, dropout_strength, train_parents=True, train_segments=True):
        while not train.epoch_finished():
            ...
            if train_parents and train_segments:
                training_target = self.training_joint
                #print("Training jointly", file=sys.stderr, flush=True)
            elif train_parents:
                training_target = self.training_parent
                #print("Training parents only", file=sys.stderr, flush=True)
            elif train_segments:
                training_target = self.training_segment
                #print("Training segments only", file=sys.stderr, flush=True)
            else:
                raise Exception("No training target specified.")

            self.session.run(self.reset_metrics)
            predictions, predicted_segments, _, _ = self.session.run(
                [self.predicted_lemmas_training, self.predicted_segments, training_target, self.summaries["train"]],
...
if __name__ == "__main__":
    ...
    parser.add_argument("--segmenter_layers", default=None, type=int, help="Layer count of the segmenter.")
    parser.add_argument("--no_train_segmentation", action="store_false", help="Don't train the segmentation model.")
    parser.add_argument("--no_train_parents", action="store_false", help="Don't train the parent prediction model.")
    ...
    for i in range(args.epochs):
        network.train_epoch(train, args.batch_size, args.dropout, args.no_train_parents, args.no_train_segmentation)
        ...
        # Predict test data
        with open("{}/segmenter_test_{}.txt".format(args.logdir, i), "w", encoding="utf-8") as test_file:
